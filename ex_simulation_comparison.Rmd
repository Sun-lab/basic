...
B-SCITE is designed for more detailed studies of a small number of mutations because it search all the possible mutation trees and the number of mutation trees increases super-exponetially with the number of mutations. Therefore analyzing 1000 mutations is computational challenging of B-SCITE. In our expereince, running B-SCITE for 1000 mutations will take more than 3 days and the resutls can be unstable across runs.  So we generate a smaller dataset by randomly sample 100 mutations. 
...
To summarize the results from BaSiC, we first generate the true clsuter membership: the first 600 mutations belong to cluster 1, 601-800 for cluster 2, 801-900 for cluster 3, and 901-1000 for cluster 4. Based on AIC, BaSiC chose 5 clusters. They are very well aligned with the 4 true clusters, except that the 1st true cluster is splitted into two clusters by BaSiC. 
```{r basic_summary}
n1 = sum(s100 <= 600)
n2 = sum(s100 <= 800) - n1
