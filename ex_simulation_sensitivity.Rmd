...
This R markdown file illustrate the usage of R package ```basic``` using simulated data, as described in our paper: Sun et al. (2019) Joint Analysis of Single Cell and Bulk Tissue Sequencing Data to Infer Intra-Tumor Heterogeneity. This is an extension of the basic simulation anlaysis in ex_simulation.Rmd to perform two sensitivity analyses. One is to adjust the rea-depth cutoff to call somatic mutations. The other is to check the sensitivity to copy number calling errors. 
...
From the summary of above analysis, both AIC and BIC suggest that there are three clusters. Next we visualize the distribution of estimated mutation status (```Ehats```) vs. the true mutation status (```E```). 
...
g2 = ggcorrplot(cd1[["n5"]]$gamma, tl.cex=10, lab=FALSE, legend.title="") + 
  ggtitle('SCS only' ) + theme(plot.title = element_text(size = 10))

gamma2 = cBSC2[["n4"]]$gamma[,c(1,2,4,3)]
...
mut2 = apply(cd1$n5$postPs, 1, which.max)
mut3 = apply(cBSC2$n4$postPs[,c(1,2,4,3)], 1, which.max)
...
match0 = (mut1==1 & mut2==2) | (mut1==3 & mut2==5) | (mut1==4 & mut2==4)
table(match0)/length(mut1)
table(mut1 == mut3)/length(mut1)
table(mut1 == mut4)/length(mut1)
```
