...
# maxIt=1000; converged=1e-5; nclusters=2:5
...
function(mutations, asrec, trec, nclusters, epsilon=0.001, 
  ...
  ## impute Ds, if there are missing values
  if(length(is.NAs) == 0){
    pi1  = rho1 = pi2 = rho2 = pi3 = rho3 = psi1 = psi2 = psi3 = NA
  }else{
  ...
  for(nclust in nclusters){
    ...
    for(j in 1:nclust){
      gammas[,j] = colMeans(Ehats[which(c1==j),,drop=FALSE])
    }
    ...
    for(i in 1:nn){
      ...
      for(k in 1:nclust){
        # likelihood for single cell data
        Lk.SC = Ehats[i,]*gammas[,k] + (1 - Ehats[i,])*(1-gammas[,k])
        tmp1[k] = log(nu[k]) + sum(log(Lk.SC))
      }
      logLikE = logLikE + logsumexp(tmp1)
    }
    ...
    AIC  = -2*logL + 2*(nclust*mm + nclust - 1)
    BIC  = -2*logL + log(nn*mm)*(nclust*mm + nclust - 1)
    # print(signif(c(logLikE, logLikDR, -2*logL, AIC, BIC)),3)
    ...
    results[[lab1]] = list(E=Ehats, theta=theta, gammas=gammas, nu=nu, 
                           postPs=postPs, logLik=logL, AIC=AIC, BIC=BIC, 
                           nIt=it, gap=gap)
  }
