...
/* ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc */
    timer1(&cpu1);
    cauchy1(n, &x[1], &l[1], &u[1], &nbd[1], &g[1], &indx2[1], &iwhere[1],
...
/*     Update matrices WS and WY and form the middle matrix in B. */
  matupd1(n, m, &ws[ws_offset], &wy[wy_offset], &sy[sy_offset],
...
*/
  ...
  for (i = 2; i <= i__1; ++i) {
    for (j = i; j <= i__1; ++j) {
      ...
      ddum = 0.;
      for (k = 1; k <= k1; ++k) {
        ddum += sy[i + k * sy_dim1] * sy[j + k * sy_dim1] /
        sy[k + k * sy_dim1];
      }
