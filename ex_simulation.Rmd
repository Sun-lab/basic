...
This R markdown file illustrate the usage of R package ```basic``` using simulated data, as described in our paper: Sun et al. (2019) Joint Analysis of Single Cell and Bulk Tissue Sequencing Data to Infer Intra-Tumor Heterogeneity. 
...
From the summary of above analayis, both AIC and BIC suggest that there are three clusters. Next we visualize the distribution of estimated mutation status (```Ehats```) vs. the true mutation status (```E```). 
...
Next we conduct similar anlaysis using the simulated data with copy number alterations.
```{r load_data_cna}
...
We compare the clustr mean estimations (```gamma```'s) versus the its true values. 
